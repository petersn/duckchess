...
use tensorflow::{Graph, SavedModelBundle, Session, SessionOptions, SessionRunArgs, FetchToken, Tensor, Operation};
...
impl InferenceEngine {
  pub async fn create() -> InferenceEngine {
    ...
    let input_tensor = Box::leak(Box::new(
      RefCell::new(
      Tensor::new(&[1, 8, 8, 22])
    ...
    let bundle = Box::leak(Box::new(SavedModelBundle::load(&SessionOptions::new(), &["serve"], &mut graph, save_dir)
    ...
    let policy_info = signature.get_output("dense").unwrap();
    let value_info = signature.get_output("dense_1").unwrap();
