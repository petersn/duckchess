...
pub struct Nnue {
  pub linear_state: [v128; LINEAR_STATE_SIZE / 4],
...
impl Nnue {
  pub fn new(state: &State) -> Self {
    let mut this = Self {
      linear_state: [i32x4_splat(0); LINEAR_STATE_SIZE / 4],
      outputs:      [0.0; 64 + 1],
      value:        0,
    };
  ...
  pub fn add_layer(&mut self, layer: u16) {
    let row: &[v128] = unsafe {
      std::slice::from_raw_parts(
        PARAMS_MAIN_EMBED_WEIGHT.as_ptr().add(layer as usize) as *const v128,
        LINEAR_STATE_SIZE / 4,
      )
    };
    for i in 0..LINEAR_STATE_SIZE / 4 {
      self.linear_state[i] = i32x4_add(self.linear_state[i], row[i]);
    }
  ...
  pub fn sub_layer(&mut self, layer: u16) {
    ...
    for i in 0..LINEAR_STATE_SIZE / 4 {
      self.linear_state[i] = i32x4_sub(self.linear_state[i], row[i]);
    }
  }
  ...
  pub fn sub_add_layers(&mut self, from: u16, to: u16) {
    let (sub_row, add_row) = unsafe {
      (
        std::slice::from_raw_parts(
          PARAMS_MAIN_EMBED_WEIGHT.as_ptr().add(from as usize) as *const v128,
          LINEAR_STATE_SIZE / 4,
        ),
        std::slice::from_raw_parts(
          PARAMS_MAIN_EMBED_WEIGHT.as_ptr().add(to as usize) as *const v128,
          LINEAR_STATE_SIZE / 4,
        ),
      )
    };
    for i in 0..LINEAR_STATE_SIZE / 4 {
      self.linear_state[i] = i32x4_add(i32x4_sub(self.linear_state[i], sub_row[i]), add_row[i]);
    }
  }
  ...
  pub fn evaluate(&mut self, state: &State) {
    ...
    for i in 0..65 {
      ...
      let mat_row = unsafe {
        std::slice::from_raw_parts(mat.as_ptr().add(i) as *const v128, LINEAR_STATE_SIZE / 4)
      };
