...
class Nnue(torch.nn.Module):
    ...
    def forward(self, indices, offsets, which_model):
...
def train(paths, device="cuda"):
    ...
    for i in range(1_000_000):
        ...
        lr = max(5e-6, 2e-4 * 0.5 ** (i / 20_000))
        model.adjust_leak(leak)
        for g in optimizer.param_groups:
            g['lr'] = lr

        optimizer.zero_grad()
        indices, offsets, which_model, value_for_white = make_batch(256)
        value_output = model(indices, offsets, which_model)
